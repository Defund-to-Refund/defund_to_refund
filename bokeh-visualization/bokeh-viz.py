...
# Data
x1 = { 'Lgbt Services': value[0] , 'Homelessness': value[1], 'Supporting POC initiatives': value[2], 'Women’s services': value[3],
    'Orphanages': value[4], 'Public libraries': value[5], 'Sanitation': value[6], 'Public Transportation ': value[7],
    'Funding Medical Healthcare Professionals': value[8]}

data = pd.Series(x1).reset_index(name='value').rename(columns={'index':'country'})
data['angle'] = data['value']/data['value'].sum() * 2*pi
data['color'] = YlOrBr[len(x1)]
